...
- **No call-outs in the app.** Time off is requested with the form at least 2 weeks ahead. For anything sooner, or a day they can't work, VAs message their management channel on Slack; the My day page links to it. Requests sent with less than 2 weeks' notice are kept (not refused), marked "Less than 2 weeks' notice" on the Time off page, and posted in **#va-lead-channel**. That channel is private, so the Slack bot must be a member: in the channel, type `/invite @VA App`.
